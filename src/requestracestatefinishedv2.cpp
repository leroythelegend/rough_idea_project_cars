...
bool Request_Race_State_Finished_V2::request(Decoder * decoder) {
	Packet * packet = dynamic_cast<Packet *>(decoder);

	if (packet) {
		if (static_cast<Race_State>(packet->timing_data().partcipants().at(0).race_state()) == Race_State::RACESTATE_FINISHED) {
