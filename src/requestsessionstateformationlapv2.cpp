...
bool Request_Session_State_Formation_Lap_V2::request(Decoder * decoder) {

	Packet * packet = dynamic_cast<Packet *>(decoder);

	if (packet) {

		if (static_cast<Session_State>(packet->game_state().sessin_state()) == Session_State::SESSION_FORMATION_LAP) {
